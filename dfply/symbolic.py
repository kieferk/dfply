...
This is taken from the pandas-ply package with some modifications for the
dfply package."""
...
class GetAttr(Expression):
    ...
    def _eval(self, context, **options):
        ...
        override_attr = options.get('override_attr', {})
        if self._name in override_attr:
            result = override_attr[self._name](evaled_obj)
        else:
...
def to_callable(obj, **options):
    ...
    if hasattr(obj, '_eval'):
        return lambda *args, **kwargs: obj._eval(dict(enumerate(args), **kwargs), **options)
    elif callable(obj):
        return obj
    else:

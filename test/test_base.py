...
def test_desc():
    ...
    assert desc(series_num)._eval(series_num).equals(series_num.rank(method='min',
                                                                     ascending=False))
    assert desc(series_bool)._eval(series_bool).equals(series_bool.rank(method='min',
                                                                        ascending=False))
    assert desc(series_str)._eval(series_str).equals(series_str.rank(method='min',
                                                                     ascending=False))
...
def test_order_series_by():
    ...
    ordered1 = order_series_by(series, order1)._eval((series, order1)).reset_index(drop=True)
    true1 = pd.Series([1,3,5,7,2,4,6,8])
    assert ordered1.equals(true1)

    order2 = pd.Series([2,2,2,2,1,1,1,1])
    ordered2 = order_series_by(series, [order1, order2])._eval((series, [order1, order2]))
    ordered2 = ordered2.reset_index(drop=True)
